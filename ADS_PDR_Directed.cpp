...
Weight calculate_new_delta(Weight min_delta, Weight current_delta) {
    Weight new_delta = max(min_delta, (Weight)ceil(current_delta/1.25)); // Placeholder for adaptive logic
    return new_delta;
}
...
pvector<Weight> delta_stepping(int p, const WCSRGraph& graph, int source, Weight delta) {
    ...
    delta = (Weight) min((Weight)ceil(1000*.15), (Weight)min_delta*10);
   ...
   #pragma omp parallel
   {
        ...
        while (shared_indexes[iter&1] != kMaxBin) {
            ...
            for (Node i=0; i < curr_frontier_tail; i++) {
                Node u = frontier[i];
                if (dist[u] >= delta * static_cast<Weight>(curr_bin_index))
            ...
            #pragma omp single nowait
            {
                ...
                Weight new_delta = calculate_new_delta(min_delta, delta);  // New adaptive delta
                if (new_delta != delta) {
                    delta = new_delta;  // Update delta
                    // Resize bins if necessary
                    vector<vector<Node>> new_local_bins;
                    for (const auto& bin : local_bins) {
                        for (const auto& node : bin) {
                            Node dest_bin = floor(dist[node] / delta);
                            if (dest_bin >= new_local_bins.size())
                                new_local_bins.resize(dest_bin + 1);
                            new_local_bins[dest_bin].push_back(node);
                        }
                    }
                    local_bins.swap(new_local_bins);
                }
...
void printDistances(vector<Weight>shortest_paths)
{
...
void comparePath(pvector<Weight> &A, pvector<Weight> &B, vector<Weight>D)
{
    ...
    cout<<"Total "<<total<<" Mismatch Found with Single thread\n"<<endl;   
}
...
int main(int argc, char *argv[]) {
    ...
    const char* suffix = "_results_PDR_ADS.txt";
    ...
    cout<<"Running for Dijkastras"<<endl;  
    shortest_paths_0 = dijkstra(graph,global_src);
    
    int it=1;
    for (Weight del= 1 ; del<=100;)
    {
